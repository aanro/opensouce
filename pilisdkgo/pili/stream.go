...
func (s Stream) Transcode() (err error) {

	msg := map[string]interface{}{"converts": []string{"720p","480p"}}

	url := fmt.Sprintf("%s/hubs/%s/streams/%s/converts",
		getSapiBaseUrl(),s.Hub,base64.URLEncoding.EncodeToString([]byte(s.Title)))

	err = s.rpc.PostCall(nil, url, msg)
	if err != nil {
		return
	}
